...
export default class extends Controller {
  ...
  add() {
    ...
    if (document.getElementById(`product-${poductId}`)) {
      ...
      priceField.innerText = price + parseFloat(productPrice);
    } else {
      ...
      document.getElementById(`input-product-${poductId}`).addEventListener('focus', (event) => {
        ...
        document.getElementById(`input-product-${poductId}`).onchange = () => {
          ...
          priceField.innerText = parseFloat(priceField.innerText) + (parseInt(currentValue, 10) - parseInt(originValue, 10)) * parseFloat(productPrice);
        }
      });
      document.querySelector(`[data-id="product-${poductId}"]`).addEventListener('click', (event) => {
        priceField.innerText = parseFloat(priceField.innerText) - (parseFloat(productPrice) * parseInt(document.getElementById(`input-product-${poductId}`).value, 10));
